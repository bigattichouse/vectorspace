...
int say_hello (void *context, void *stack, float value){
    printf("Hello! %f\n",value);
    return 1;
}

int say_goodbye (void *context, void *stack, float value){
    printf("Bye! %f\n",value);
    return 1;
}


int third_fn (void *context, void *stack, float value){
    printf("Third Function Auto added and shows in the stack! (value: %f)\n",value);
    return 1;
}

int second_fn (void *context, void *stack, float value){
    printf("Second Function Auto added and shows in the stack! (value: %f)\n",value);
    return 1;
}

int first_fn (void *context, void *stack, float value){
    printf("Adding two more items to the stack\n");
    fn_stack_push((fn_stack *)stack,context, &second_fn, 4.0);
    fn_stack_push((fn_stack *)stack,context, &third_fn, 5.0);
    return 1;
}
...
int main(){
    ...
    sz = fn_stack_push(stack, v, &say_hello, 1.1);
    ...
    printf("Stack Now has %d items.\n",sz);
    printf("Execute fn: \n");
    ref->fn(ref->context,ref->stack, ref->value);

    printf("Push 3 to stack (hello, hello, goodbye)\n");
    sz = fn_stack_push(stack, v, &say_hello, 1.1);
    printf("Stack Now has %d items.\n",sz);
    sz = fn_stack_push(stack, v, &say_hello, 2.2);
    printf("Stack Now has %d items.\n",sz);
    sz = fn_stack_push(stack, v, &say_goodbye, 3.3);
    printf("Stack Now has %d items.\n",sz);
    ...
    sz = fn_stack_push(stack, v, &first_fn, 3.0);
    sz = fn_stack_size(stack);
    printf("Stack Now has %d items.\n",sz);
