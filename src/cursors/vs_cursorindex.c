...
ROWSET read out - keep a vslist (null polongers, just id's... ), if we've already done a vector, then skip !  hah!
...
long __cdim_dimension_frequency (__cdim_index *index, t_uuid dimensionid){
   __cdim_node *node;
   vs_list *vectors;
   node = __cdim_find_dimension_node (index->root, dimensionid);
   if(node==NULL){
      return(0);
   } else {
        vectors = node->vectors;
        return(vs_list_size(vectors));
   }
}
