...
int sock;            /* The socket file descriptor for our "listening"
                   	socket */
const int TOTALSOCKS = 15;
vs_connection connectlist[15];  /* Array of connected sockets so we know who
	 		we are talking to */
...
void process_command (vs_connection *connection){
 ...
 if (sscanf(connection->buffer,"%c %32s %f\r\n",&command,vectorid,&value)==3){
    ...
    res = vsd_interpreter_execute (intf, "",cursor,thesaurus,&connection->session,command,id,value, connection->output_buffer);
...
void deal_with_data( int listnum) {
    ...
    buff = (char *)((int)connection->buffer + connection->bufferpos);
...
void open_logfile(){
  ...
  xlogfile=freopen("/var/log/vsdbd/vsdbd.log", "a+",logfile); //force line buffered
